...
Page({
  ...
  onUnload() {
    ...
    if (!room) return
    const realPlayers = (room.players || []).filter(p => p.id !== '__tea__')
  ...
  // === Tap Player: open pay dialog (dev: any player can pay any player) ===
  ...
  drawChart() {
    ...
    const players = room.players.filter(p => p.id !== '__tea__')
